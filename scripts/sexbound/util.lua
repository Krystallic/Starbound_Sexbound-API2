...
local function Sexbound_Util_ResetMessenger(message)
...
Sexbound.Util.sendMessage = function(entityId, message, args, wait)
  ...
  if (self.messenger[message] == nil) then
    Sexbound_Util_ResetMessenger(message)
  end
...
-- @param message String: The message to send the remote entity.
-- @param callback
Sexbound.Util.updateMessage = function(message, callback)
  if (self.messenger == nil) then self.messenger = {} end
